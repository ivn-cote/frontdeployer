 ...
 * @param {Array.<{outputPath:string,templateFile:string}>} mapFiles
 * @constructor
 */
var CssObfuscator = module.exports = function(cssFilePath, mapFiles) {
  ...
  /**
   * @type {Array.<{outputPath:string,templateFile:string}>}
   */
 ...
 *    outputPath: string
 *    mapFiles: Array.<{outputPath:string,templateFile:string}>
 * @param {function(Error)=} opt_callback
 */
CssObfuscator.createFromConfig = function(options, opt_callback) {
  var cssMap = new CssObfuscator(options.outputPath, options.mapFiles);
